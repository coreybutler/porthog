...
class Parser {
  ...
  static ss (port = '') {
    ...
    stdout.forEach(row => {
      // Group 1 is port
      let data = /:{1,3}([A-Za-z0-9]+)/i.exec(row)
      ...
      if (port.indexOf(data[1].toString()) >= 0) {
        let ct = 0
        let PID = /\(\"([A-Za-z]+)\".*\,[pid=]{0,4}([0-9]+)\,/i.exec(data.input) // eslint-disable-line no-useless-escape
        ...
        while (PID !== null && ct < 100) {
          ...
          PID = /\(\"([A-Za-z]+)\".*pid=([0-9]+)/i.exec(data.input) // eslint-disable-line no-useless-escape
          ct++
        }
      }
    })
  ...
  static linuxProcessDetails (pid) {
    let processDetail = exec('ps axco pid,user,command | grep ' + pid)
      .toString()
      .split('\n')
    ...
    let title = processDetail.length === 3 ? processDetail[2] : processDetail.slice(2).join(' ')
